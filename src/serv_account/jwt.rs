...
impl JwtToken {
    ...
    pub fn to_string(&self) -> Result<String> {
        ...
        // Google requires standard base64url-safe encoding with no padding
        let base64_header = URL_SAFE_NO_PAD.encode(header);
        let base64_payload = URL_SAFE_NO_PAD.encode(payload);

        let raw_signature = format!("{}.{}", base64_header, base64_payload);
        let signature = self.sign_rsa(raw_signature)?;

        let base64_signature = URL_SAFE_NO_PAD.encode(signature);
    ...
    fn sign_rsa(&self, message: String) -> Result<Vec<u8>> {
        ...
        // Decode key from JSON PEM structure using standard Base64
        let decoded = STANDARD.decode(private_key)?;

        let key_pair = signature::RsaKeyPair::from_pkcs8(&decoded).map_err(|err| {
            ServiceAccountError::RsaKeyPair(format!("failed to create key pair: {}", err))
        })?;
